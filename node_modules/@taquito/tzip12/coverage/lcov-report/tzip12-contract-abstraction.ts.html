...
<head>
    <title>Code coverage report for tzip12-contract-abstraction.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> tzip12-contract-abstraction.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { MichelsonMap, Schema } from '@taquito/michelson-encoder';
import { BigMapAbstraction, ContractAbstraction, ContractProvider, Wallet } from '@taquito/taquito';
...
&nbsp;
    ...
    async getTokenMetadata(tokenId: number) {
        ...
        return <span class="missing-if-branch" title="if path not taken" >I</span>(!tokenMetadata) ? await this.retrieveTokenMetadataFromBigMap(tokenId) : <span class="cstat-no" title="statement not covered" >tokenMetadata;</span>
    }
&nbsp;
...
&nbsp;
    private hasTokenMetadataView(views: {}) {
        for (let view of Object.keys(views)) {
            <span class="missing-if-branch" title="else path not taken" >E</span>if (view === 'token_metadata') {
                return true;
            }
        }
        return false;
    ...
    private async fetchTokenMetadataFromUri(tokenMetadata: MichelsonMap&lt;string, string&gt;) {
        ...
        if (uri) {
            try {
                ...
                <span class="missing-if-branch" title="else path not taken" >E</span>if (e.name === 'InvalidUri') {
...
&nbsp;
    private formatMetadataToken(metadataTokenMap: MichelsonMap&lt;string, string&gt;): TokenMetadata {
        ...
        for (let keyTokenMetadata of metadataTokenMap.keys()) {
            ...
            else <span class="missing-if-branch" title="else path not taken" >E</span>if (!(keyTokenMetadata === '')) {
